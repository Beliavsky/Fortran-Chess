...
echo.
echo This script will force-push and can overwrite the remote branch history.
echo.
...
if defined WORKTREE_DIRTY (
    choice /M "Commit the current working tree before force-pushing"
    if errorlevel 2 goto skip_commit

    set "COMMIT_MESSAGE=Force-sync local repo to Beliavsky/Fortran-Chess"
    set /p "COMMIT_MESSAGE=Commit message [Force-sync local repo to Beliavsky/Fortran-Chess]: "
    if "!COMMIT_MESSAGE!"=="" set "COMMIT_MESSAGE=Force-sync local repo to Beliavsky/Fortran-Chess"
...
choice /M "Proceed with FORCE push to %TARGET_URL%"
if errorlevel 2 (
...
echo.
echo Force-pushing %CURRENT_BRANCH% to %TARGET_REMOTE% as %TARGET_BRANCH%...
git push -u %TARGET_REMOTE% HEAD:%TARGET_BRANCH% --force
if errorlevel 1 (
